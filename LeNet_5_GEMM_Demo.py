...
# [기능] 희소 행렬(Sparse Matrix)을 하드웨어 친화적 포맷(CSC)으로 변환
# 0이 아닌 값(Data), 그 값의 위치(Count), 열의 시작점(Address)을 추출
def genAdrCountData(listA, inActOrWeight, modified_mask):
    if inActOrWeight:
        zeroCode = 255 # 빈 열(Column)을 나타내는 특수 코드
        max_count = 31 # Activation의 행 인덱스 최대값 (5비트)
    else:
        zeroCode = 127
        max_count = 15 # Weight의 행 인덱스 최대값 (4비트)

    adrList = [] # 주소 리스트 (포인터 역할)
    countList = [] # 카운트 리스트 (행 인덱스)
    dataList = [] # 실제 데이터 값 리스트

    for j in range(len(listA[0])): # 행렬의 각 열(Column)을 순회
        currentCol = [row[j] for row in listA] # 현재 열 데이터 추출
        currentMaskCol = [row[j] for row in modified_mask]
        
        # 만약 현재 열의 모든 값이 0이라면?
        if max(map(abs, currentCol)) == 0:
            adrList.append(zeroCode) # 데이터 없음 표시(zeroCode) 추가
        else:
            # 첫 번째 열이 아니라면, 현재까지 쌓인 데이터 개수를 주소로 저장
            if j != 0:
...
# [기능] 이미지(2D)를 커널이 지나가는 순서대로 펴서 행렬(1D 나열)로 만듦
# 이렇게 하면 복잡한 슬라이딩 연산 없이 단순 행렬 곱으로 합성곱 처리가 가능함
def im2col(input_array, kernel_size):
    # Extracting the dimensions of the input array
    input_height, input_width = input_array.shape

    # Output dimensions
    output_height = input_height - kernel_size + 1 # 출력 크기 계산
    output_width = input_width - kernel_size + 1

    # 변환될 행렬 초기화 (행: 픽셀 수, 열: 커널 크기 제곱)
...
class App(tk.Tk):
    ...
    def preprocess(self):
        # Image.ANTIALIAS를 Image.Resampling.LANCZOS로 수정
